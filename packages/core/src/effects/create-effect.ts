 ...
 *
 * Effects can be used to interact with an atom by using the following lifecycle
 * actions:
 *
 * - `init`: Action to be called when the atom is created, but before subscribing
 *   to `set` events. May return a promise that can be awaited by using
 *   `atom.didInit`.
 * - `didInit`: Action to be called when the atom is created, but after
 *   subscribing to `set` events. May return a promise that can be awaited by
 *   using `atom.didInit`.
 * - `set`: Action to be called when the atom's value is set.
 *
 * @param setup Effect actions or function to create effect actions. Effect
 *   actions are fired in the atom lifecycle, alongside to the subscriptions.
 *   Additionally, you can pass a `sort` option, which will enforce a position
 *   of the effect in the execution order.
 *
